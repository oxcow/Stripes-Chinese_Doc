...
现在你有了自己的`ActionBeanContext`对象，接下来需要告诉Stripes如何去使用它。最简单的方式是将其放在扩展包中（请看[扩展](2016-07-10-stripes-how-to-extensions.md)一文）。另外，你可以在web.xml中为Stripes过滤器添加如下的初始参数：
...
现在，被设置为你的ActionBean的ActionBeanContext将总是被当做MyAppActionBeanContext。你仍然需要对那些在你的ActionBean中接收到的对象做类型转换，但是幸运的是在Java1.5中有一个叫做[协变返回类型](https://blogs.oracle.com/sundararajan/entry/covariant_return_types_in_java)的新属性，你可以至少使用该属性一次，而不是通过类型强制转换代码。
...
虽然这看起来微不足道，但是有一个好处。第一个语法`${user｝`假设你已经通过属性关键字“user”将用户对象放置到JSP范围中（比如会话中）。如果你修改了这个关键字，比如改成“__secretPlace_user”，那么JSP将会阻断。你大概不太可能去改变	`ActionBeanContext`中类型安全的方法，因此`${actionBean.context.user}`可能更安全。这些虽然微不足道，但值的一提。

### 可测试

对于web层面的类进行测试通常来说是困难的。这通常是因为它们不仅需要和以Http开头的对象相互作用，还被迫需要通过Servlet、Action或者其他模型的API进行交互。Stripes ActionBeans的单元测试已经是非常简单的了（仅仅需要初始化，设置上下文，设置属性然后继续），但是当你的ActionBean真的需要和请求或者会话交互时，你的单元测试将无法继续。因此你可能不得不去构造一个假的Http对象等。

只需要多一点的工作，就可以让`ActionBeans`完全可测试化。使用上面提到的`MyAppActionBeanContext`做示例，我们可以创建一个抽象类，而我们的`ActionBeanContext`将会继承它，同时我们的`ActionBeans`也将用到它。让我们重构下：
...
这样，我们就拥有了一个自定义ActionBean的测试类，使的能通过很少的努力就能模拟真实环境。你可以想象下其他处理方式。获取测试版本（指`MyAppActionBeanContextTestImpl`）可以简单地继承“真”版本（指`MyAppActionBeanContextImpl`）并且重载访问会话或者请求的方法。你甚至可以进一步在你的“真实”上下文类中定义一个被叫做类似`setInSession(String key, Object value),`的受保护的助手方法，这样在真实版本中调用`getRequest().getSession().setAttribute()`，而在测试版本中仅将值存放在本地`Map`中。
...
但是对于重定向时有不好的一面。当重定向发出时，任何存储在请求中的属性值会丢失（因为对于页面的请求和当前请求不是同一个请求）。一种选择是将需要的属性值存放在会话中，但这是一个不好的主意。因为当用户使用多个浏览器打开应用时，正式方式会被破坏掉。

### FlashScope

[FlashScope]被设计用来解决这个问题。本质上，闪存范围是类似请求范围、会话范围等的。而请求范围是被定义为当前请求的生命周期而存在，闪存范围则被定义为当前请求和后续请求的生命周期而存在。任何闪存范围内的添加的项，都将被当做当前请求以及下一个请求的请求属性。

一个闪存范围用法的示例是Stripes中的非错误消息功能。[ActionBeanContext]有一个`getMessages()`的方法，它返回可能被添加的消息列表。这个消息列表存放在闪存范围中以至于消息对于当前请求是可用的（应该`ActionBean`转发到一个页面）并且在下一个请求同样可用（应该`ActionBean`重定向到一个页面）。代码看起来如下：
...
第一行真实的调用`FlashScope`的`getCurrent`创建一个`FlashScope`对象，假如`FlashScope`不存在时。接着像其他范围一样，通过关键字／值来使用新建的`FlashScope`对象。

### 闪存中的ActionBeans

默认情况下不把`ActionBeans`存放到闪存范围中。这部分出于向后兼容的原因，部分是因为没有必要任何时候都这样做！而不是说他做起来很困难。假如使用标准的[RedirectResolution]，它是简单的通过链路方法来调用：

	return new RedirectResolution("/some/page.jsp").flash(this); 

甚至，假如你要重定向一些其他的方法，它仍然是非常简单的：
...
如你所看到的，当你存放一个对象在`FlashScope`中，它可以通过关键在EL表达式中被直接访问。

### FlashScope是如何工作的

`FlashScope`的是通过临时存储自己的实例在会话中并在随后的请求中移除它们的方式工作的。结果是，当`FlashScope`被用时，一个额外的变量被追加到重定向URL中，用来告诉Stripes使用那个`FlashScope`。－－－－

这种处理方式有一个明显的副作用，那就是在会话中偶尔会得到。

对于这种情况，Stripes会在每次收到一个请求时，针对会话检查存在的所有`FlashScope`；当生成闪存范围的请求完成时，每一个闪存范围开始“老化”，同时任何超过一定年龄（当前是２分钟）的将被销毁。这个看起来很想一个小范围内的会话过期。
